...
if upload_file is not None:
    ...
    if st.sidebar.button('Show Details'):
        ...
        st.image(resized_image, caption='Image', clamp=True)
        ...
        st.pyplot(fig)
        st.markdown('#### <span style="color: purple;">STRANGE `\U0001F615`: Inbetween 2 and 3 there is rare any '
                    'chat, lagtha hai class lekr thak jate hai `\U0001F923` `\U0001F923` .</span>',
                    unsafe_allow_html=True)
        ...
        fig.update_layout(title=f'Group Chat Activity for {selected_user}',
                          ...
                          yaxis_title='Month',
                          showlegend=True,
                          legend=dict(font=dict(color='brown')),  # Change legend text color to black
        ...
        if selected_user == 'Overall Group':
            st.title('Top 20 Chatters by Message Count for Each Month')
